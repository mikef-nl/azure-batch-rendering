...
def main():
    ...
    if args.operation.lower() == "test":
        ...
        print("unrecognised run operation: '{0}', exiting".format(args.operation))
...
def _parse_runner_arguments():
    ...
    parser = argparse.ArgumentParser(description="Submit a collection of jobs to Azure Batch") 
    parser.add_argument("--manifest", type=str, help="Manifest file location.")
    parser.add_argument("--operation", nargs="?", type=str, default="run", help="What operation to run? Can choose 'run' or 'test'.")
    parser.add_argument("--pool", nargs="?", type=str, help="Pool name to run the jobs on. At this stage, it must already exist.")
    ...
    if args.manifest == None:
        raise Exception("manifest file was not supplied on the command line. application cannot continue.")
...
def _load_job_manifest(args: object) -> Tuple[AuthSettings, TemplateSettings, list, str]:
    ...
    if args.pool != None:
        ...
        raise Exception("No pool was defined in either the manifest or the command line. application cannot continue.")
...
def _test_job(job: Job, job_config: JobConfiguration): 
    ...
    """
    print("testing job: {} - scene file: {}, container: {}, folder: {}".format(job.job_id, job.scene_file, job.container, job.folder))

    try:
        job_exists = batchapi.does_job_exist(job_config.batch_client, job.job_id)
        if job_exists == True:
            print("WARN :: job with id '{}' already exists. Job definition cannot be submitted.".format(job.job_id))
        else:
        ...
        if scene_exists == True: 
           ...
           print("WARN :: container '{}' and or scene file: {} does not exist. job definition cannot be submitted.".format(job.container, job.scene_file))

    except Exception:
        # error has already been logged
        print("ERROR :: Failed to request job: '{}' or scene file: '{}/{}'".format(job.job_id, job.container, job.scene_file))
...
def _run_manifest(jobs: [Job], job_config: JobConfiguration):
    ...
    """
    try:
        abs_path = os.path.abspath(job_config.template_settings.parameter_mapper_file_path)     
        print("loading parameter mapper module: ", abs_path)   
    
        # dynamically load the custom module for mapping job parameters
        spec = importlib.util.spec_from_file_location(job_config.template_settings.mapper_module_name, abs_path)
    ...
    print("about to submit '{}' jobs to '{}'".format(len(jobs), job_config.batch_client.config.base_url))
    for job in jobs:
        try:
...
def _submit_job(job: Job, job_config: JobConfiguration, mapper: object): 
    ...
    try:
        job_exists = batchapi.does_job_exist(job_config.batch_client, job.job_id)
        if job_exists == True:
            print("ALERT :: job already exists with id: {}, skipping.".format(job.job_id))
            return
        ...
        print("caught generic exception while submitting job: '{}'. with error:".format(job.job_id), ex)
